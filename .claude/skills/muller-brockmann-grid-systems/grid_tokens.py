...
editorial/magazine webpage on a REAL, VISIBLE, VERIFIED modular grid:
  ...
  • The grid-toggle OVERLAY reads the SAME variables and lives in the SAME
    ...
    (this is the fix for the "grid is just slapped on top / misaligned"
    bug that happens when the overlay is a full-width sibling of a
    centered max-width container).
  • Subgrid "bands" so every element is placed by column LINE, not
    eyeballed.
  • Vertical rhythm locked to an 8px baseline (24px leading).
  • Runtime OPTICAL ALIGNMENT: display type is nudged so its INK (not its
    box) lands on the column line — large letterforms carry a left
    side-bearing, so a headline whose box is on the grid still looks
    misaligned vs body text.

No network, no credentials. Deterministic.

Usage:
  python3 grid_tokens.py                      # print CSS + JS block
  python3 grid_tokens.py --scaffold           # print a full minimal HTML
...
/* ---- vertical rhythm helpers (keep ALL spacing a multiple of --bl) ----
   line-heights for display type MUST be px multiples of --bl, never
   unitless, or the box height drifts off the baseline. Media heights =
   multiples of --lh so photo top AND bottom land on lines. */
...
/* ---- OPTICAL ALIGNMENT --------------------------------------------------
   Large display glyphs carry a left side-bearing: the ink sits inside
   the layout box, so a headline whose BOX is on the column line still
   LOOKS indented (or overhangs) vs body text. Measure each display
   glyph's actual ink offset and nudge the element so its visible ink
   lands on the line. Scales with fluid type; re-runs after the webfont
   loads and on resize. Add the selector list to match your display
   elements. */
(function(){
