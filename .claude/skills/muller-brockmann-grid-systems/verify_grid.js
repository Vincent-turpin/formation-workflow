 ...
 *      CAVEAT: side-bearing is font-specific. In a sandbox the webfont is
 *      often absent and canvas falls back to a different grotesque (we
 *      measured -16px fallback vs -7px real Inter). To verify optics offline,
 *      EMBED the real webfont via @font-face(local TTF). In production the
 *      page's runtime JS measures the actually-loaded font, so it is correct
 *      for the user.
 *
 * Env / args:
